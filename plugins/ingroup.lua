...
local function setowner_by_reply(extra, success, result)
  ...
  data[tostring(msg.to.id)]['set_owner'] = tostring(msg.from.id)
      save_data(_config.moderation.data, data)
      savelog(msg.to.id, name_log.." ["..msg.from.id.."] s ["..msg.from.id.."] as owner")
...
if msg.to.type == 'chat' then
 ...
 --[[Experimental
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[1] == 'sname' and is_momod(msg) then
    ...
    if matches[1] == 'sphoto' and is_momod(msg) then
      data[tostring(msg.to.id)]['settings']['set_photo'] = 'waiting'
      save_data(_config.moderation.data, data)
      return 'Please send me new group photo now'
    end
    if matches[1] == 'pro' and not matches[2] then
	   if not is_momod(msg) then
        return
      end
      if not is_owner(msg) then
    ...
    if matches[1] == 'pro' and matches[2] then
      if not is_momod(msg) then
        return
      end
      if not is_owner(msg) then
...
if msg.to.type == 'chat' then
    ...
    if matches[1] == 'owner' then
      local group_owner = data[tostring(msg.to.id)]['s_owner']
    ...
    if matches[1] == 'sgpowner' then
      local receiver = "chat#id"..matches[2]
      if not is_admin1(msg) then
        return "For admins only!"
      end
      data[tostring(matches[2])]['s_owner'] = matches[3]
      save_data(_config.moderation.data, data)
      local text = matches[3].." added as owner"
      send_large_msg(receiver, text)
      return
    end
    if matches[1] == 'sflood' then
      if not is_momod(msg) then
        return "For moderators only!"
      end
...
return {
  ...
  "^[#!/](owner)$",
  "^[#!/](res) (.*)$",
  "^[#!/](sgpowner) (%d+) (%d+)$",-- (group id) (owner id)
  "^[#!/](unlock) (.*)$",
  "^[#!/](sflood) (%d+)$",
  "^[#!/](settings)$",
  "^[#!/](public) (.*)$",
